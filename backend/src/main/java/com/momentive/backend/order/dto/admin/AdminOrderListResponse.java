...
/**
 * 관리자 주문 목록 응답. 결과가 없어도 {@code content}가 빈 배열인 바디를 항상 내려보낸다
 * (빈 바디를 파싱하려다 터지는 계약 불일치를 만들지 않기 위함).
 */
public record AdminOrderListResponse(
        @Schema(description = "주문 목록") List<AdminOrderSummaryResponse> content,
    ...
    public static AdminOrderListResponse from(Page<AdminOrderSummaryResponse> page) {
