...
- [ ] 주문이 `PAID`로 전환되면 `shippingStatus`가 자동으로 `PREPARING`("배송준비중")이 된다
- [ ] `GET /admin/orders`가 파라미터 없이 호출되면 `PAID` 상태 주문만 반환한다
- [ ] `GET /admin/orders`에 `status=PENDING,PAID,FAILED,CANCELLED`를 넘기면 전체 상태 주문이 반환되고, 페이지네이션이 정상 동작한다
- [ ] `GET /admin/orders/{orderId}`가 배송상태·택배사·송장번호·주문자 정보를 포함해 반환한다. 존재하지 않는 주문ID는 404(`ORDER_NOT_FOUND`)
- [ ] `PATCH /admin/orders/{orderId}/shipping`으로 `shippingStatus=SHIPPING`을 택배사·송장번호 없이 요청하면 400(`SHIPPING_INFO_REQUIRED`)이 반환되고 상태가 바뀌지 않는다
- [ ] `PATCH /admin/orders/{orderId}/shipping`으로 `shippingStatus=DELIVERED`를 택배사·송장번호 없이 요청해도 동일하게 400(`SHIPPING_INFO_REQUIRED`)이 반환된다
- [ ] `PAID`가 아닌 주문(`PENDING`/`FAILED`/`CANCELLED`)에 `PATCH .../shipping`을 요청하면 400(`ORDER_SHIPPING_NOT_APPLICABLE`)이 반환된다
- [ ] 이미 `DELIVERED`인 주문을 `SHIPPING`으로(또는 그 반대로) 되돌리는 요청이 성공한다 — 순서 제약이 없다
- [ ] `POST /admin/orders/{orderId}/cancel`이 `PAID` 주문에 대해 성공하고, 재고가 고객 취소와 동일하게 복원되며 주문이 `CANCELLED`로 전이된다
- [ ] `POST /admin/orders/{orderId}/cancel`이 `PAID`가 아닌 주문에 대해 409(`ORDER_NOT_CANCELLABLE`)를 반환한다
- [ ] 관리자가 아닌 로그인 사용자가 `/admin/orders/**` 엔드포인트를 호출하면 403(`FORBIDDEN`)이 반환된다
- [ ] 고객용 `GET /orders/{orderId}` 응답에 `shippingStatus`/`courier`/`trackingNumber`가 포함되고, `mypage/orders/[orderId]` 화면에 배송상태 뱃지가 표시된다(값이 없으면 뱃지 미노출)
- [ ] `mypage/orders/[orderId]`에 송장번호가 있으면 택배사명과 함께 표시된다
- [ ] `/admin/orders` 화면에서 기본 필터가 `PAID`만 보여주고, "전체보기" 토글로 전체 상태 주문을 볼 수 있다
- [ ] `/admin/orders/[orderId]`에서 택배사 드롭다운의 "기타" 선택 시 자유 텍스트 입력이 노출되고, 저장 시 그 값이 `courier`로 전송된다
- [ ] `/admin/orders/[orderId]`에서 대상 주문이 `PAID`가 아니면 배송 관리 폼과 취소 버튼이 비활성화된다
- [ ] `V17` 마이그레이션 적용 후 기존 `PAID` 주문의 `shipping_status`가 `PREPARING`으로 백필돼 있다(로컬 DB에서 SQL로 직접 확인)
