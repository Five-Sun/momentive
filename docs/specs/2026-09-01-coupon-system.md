...
- [ ] 존재하지 않는 코드 / 만료된 쿠폰 / 이미 등록한 코드가 각각 다른 문구로 입력칸 아래 인라인 표시된다 (존재하지 않는 코드/이미 등록한 코드는 E2E로 검증됨. 만료 쿠폰은 로컬 시드에 만료 쿠폰이 없고 관리자 발급 API도 없어 검증 못함)
...
- [ ] 정률 쿠폰의 할인액이 `maxDiscountAmount`를 넘지 않는다 (E2E에서 정률 쿠폰 상한 초과 케이스를 직접 적용해 검증하지 않음, Phase 2 백엔드 테스트로 커버)
...
- [ ] `./gradlew build` / `./gradlew test` / `npm run build` / `npm run lint` 전부 통과한다 (이번 E2E 검증에서 재실행하지 않음, Phase 1~4에서 이미 통과 확인됨)
