...
- [ ] 마이페이지 "쿠폰함"을 누르면 `/mypage/coupons`로 이동한다
- [ ] 유효한 쿠폰 코드를 입력하면 "사용 가능한 쿠폰" 목록에 즉시 추가된다
- [ ] 존재하지 않는 코드 / 만료된 쿠폰 / 이미 등록한 코드가 각각 다른 문구로 입력칸 아래 인라인 표시된다
- [ ] 같은 코드를 같은 계정으로 두 번 등록할 수 없다 (DB unique 제약으로도 보장)
- [ ] 쿠폰함에서 사용 완료·만료 쿠폰이 사용 가능 쿠폰과 구분되어 아래쪽에 표시된다
- [ ] 장바구니 화면에 쿠폰 관련 UI가 더 이상 존재하지 않고 `COUPON_DISCOUNT` 상수가 코드에서 제거되었다
- [ ] 체크아웃에서 보유 쿠폰이 전부 나열되고, 최소 주문금액 미달 쿠폰은 선택 불가 상태로 사유와 함께 표시된다
- [ ] 체크아웃에서 쿠폰을 선택/해제하면 "쿠폰 할인" 줄과 총 결제금액이 즉시 갱신된다
- [ ] 정률 쿠폰의 할인액이 `maxDiscountAmount`를 넘지 않는다
- [ ] 할인액이 상품금액보다 큰 경우 상품금액까지만 적용되어 결제금액이 배송비 이상으로 유지된다
- [ ] 무료배송 임계값(70,000원) 판정이 **할인 전** 상품금액 기준으로 이루어진다 — 쿠폰 적용 후 금액이 임계값 아래로 내려가도 배송비가 붙지 않는다
- [ ] 주문 생성 시 서버가 쿠폰 소유·상태·유효기간·최소 주문금액을 재검증하고, 위반 시 400으로 거부한다
- [ ] 주문 생성 후 해당 쿠폰이 선점되어 다른 주문에 다시 사용할 수 없다
- [ ] 결제 실패 시 쿠폰이 사용 가능 상태로 복원된다
- [ ] `PENDING` 주문이 만료되면 재고와 함께 쿠폰도 복원된다
- [ ] `PAID` 주문을 취소하면 재고와 함께 쿠폰도 복원된다
- [ ] Toss 결제위젯이 할인이 반영된 최종 금액으로 렌더되고, confirm 금액 검증 로직은 변경 없이 통과한다
- [ ] 마이페이지 주문상세에 "쿠폰 할인 −N원"과 쿠폰명이 표시되고, 쿠폰 미사용 주문에서는 해당 줄이 렌더되지 않는다
- [ ] 기존 주문(마이그레이션 백필 대상)의 주문상세 금액 내역이 깨지지 않고 그대로 표시된다
- [ ] `./gradlew build` / `./gradlew test` / `npm run build` / `npm run lint` 전부 통과한다
