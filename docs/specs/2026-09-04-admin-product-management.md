...
- [x] 기존 `order_item` 행의 `size` 문자열이 그대로 남아 있고 주문 상세 화면이 정상 표시된다
...
- [x] `POST /admin/images/signature`가 유효한 Cloudinary 서명을 반환하고, API secret이 응답에 포함되지 않는다

### 검색

- [x] `GET /products?q=...`가 `name` 부분일치로 검색하고 페이지네이션이 동작한다
- [x] `q`와 `category`, `sort`를 함께 지정하면 모두 적용된다
- [x] 상품을 101개 이상 등록한 상태에서 101번째 이후 상품이 검색된다
- [x] `/search` 화면이 서버 검색으로 동작하고 자동완성도 같은 API를 사용한다
- [x] 검색 API 실패 시 "검색 결과가 없어요"와 구분되는 실패 안내가 표시된다
...
### 미검증 항목 (2026-09-07 기준, 35/35 — 모두 체크 완료)

아래 4건은 2026-09-07에 로컬에서 직접 재검증하고 체크 완료했다.

- **기존 `order_item.size` 보존 / 주문 상세 정상 표시** — 로컬에 마이그레이션 이전 주문이 실제로 존재하지 않아, 정상 주문을 하나 생성한 뒤 그 `order_item.variant_id`만 `NULL`로 직접 갱신해 "구 데이터 형태"(size 문자열은 보존, variant_id만 없음 — `V16` 마이그레이션이 만드는 모양과 동일)를 재현했다. API(`GET /orders/{id}`)와 `/mypage/orders`, `/mypage/orders/[orderId]` 화면 모두 에러 없이 "사이즈 S · 수량 1개"를 정상 표시함을 확인(검증 후 테스트 주문/유저는 삭제). **다만 이는 시뮬레이션이며 실제 운영 DB의 마이그레이션 이전 주문으로 검증한 것은 아니다 — 운영 배포 직후 실제 과거 주문이 있다면 한 번 더 확인할 것을 권장한다**
- **상품 101개 이상에서 101번째 이후 검색** — 관리자 API로 테스트 상품 105개를 시드해 `GET /products?q=...`와 `/search` 화면 양쪽에서 101번째 이후 등록 상품(`SearchCapTest-105`)이 정상 검색·렌더링됨을 확인(검증 후 시드 데이터는 삭제)
- **검색 API 실패 안내** — dev-browser로 `/products` 요청을 강제 실패시켜 "검색에 실패했어요" + "다시 시도" 버튼이 뜨고 "검색 결과가 없어요"(빈 결과)와 명확히 구분됨을 확인
- **Cloudinary 서명 유효성** — 실제 Cloudinary 계정 자격증명(`MOMENTIVE_CLOUDINARY_*`)으로 백엔드를 재기동해 `POST /admin/images/signature` → 발급된 서명으로 실제 이미지 업로드까지 end-to-end로 성공 확인(`secure_url` 반환, 지정 폴더에 저장됨, 응답에 secret 미노출). 검증 후 테스트 업로드 파일은 Cloudinary에서 삭제
