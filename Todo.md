...
### 열려 있는 PR — 이 순서로 머지할 것

| PR | 내용 | 브랜치 |
|---|---|---|
| [#16](https://github.com/Five-Sun/momentive/pull/16) | Todo.md 재구성 + 적립금 메뉴 숨김 | `chore/todo-restructure` |
| [#17](https://github.com/Five-Sun/momentive/pull/17) | 1단계 — 결제 완료 후 장바구니 미비움 + 배송비 문구 정정 | `fix/checkout-cart-and-shipping-notice` |

**#17은 #16 위에 쌓은 브랜치다.** #16을 먼저 머지해야 #17의 diff에 해당 변경만 남는다(#17이 Todo.md 체크박스를 갱신하기 때문에 분리할 수 없었다). 두 PR 모두 `npm run lint`/`build` 통과 상태이며 미푸시 커밋은 없다.

### 다음에 할 일

1. **#16 → #17 순서로 머지**, 머지 후 로컬 브랜치 2개 정리
2. **`chore/admin-login-seed` 브랜치 행방 확인** — 2단계 착수 전 반드시 선행. 이 Windows 머신에도 origin에도 없고 Mac에만 커밋된 채 남아있을 가능성이 크다. admin 로그인 + `V9__seed_admin_user.sql`을 포함해 2단계와 정면으로 겹친다. 살릴지 버릴지 정하고, 살리면 마이그레이션을 `V13`으로 재번호
3. **`/grillme 관리자 기능`** — 상품 CRUD · 검색 API · 재고 모델(사이즈별 여부) · 배송상태/송장 · 쿠폰 발급 5가지를 인터뷰. 하나의 spec으로 갈지 쪼갤지는 세션 중 판단

### 남겨둔 검증 (Toss 상점 등록 후 반드시 함께)

1단계에서 고친 "결제 완료 후 장바구니 정리"는 `/checkout/success`에 도달해야 확인 가능한데, 상점 미등록으로 실결제 경로를 탈 수 없어 **브라우저 검증을 못 했다**. 상점 등록이 끝나면 Toss 실연동 검증과 **같은 세션에서** 부분결제 시 결제한 항목만 사라지는지까지 확인할 것.
