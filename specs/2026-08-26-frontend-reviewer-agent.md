...
status: confirmed
supersedes: 2026-08-25-frontend-reviewer-agent.md
---
...
혼자 개발 + AI 에이전트 팀 체제에서, 매 phase 구현이 끝날 때마다 사용자가 직접 코드를 전부 눈으로 검토하지 않아도 "올바른 시야"(spec의 수용기준, plan의 step, 프론트엔드 컨벤션)로 검증해주는 존재가 필요하다. 이전 spec(2026-08-25)으로 에이전트를 만들고 Phase 2 dry-run으로 실호출 검증하는 과정에서, `ReportFindings` 툴이 커스텀 서브에이전트에는 실제로 부여되지 않는 플랫폼 제약을 발견했다. 이 spec은 그 발견을 반영해 보고 방식을 대화형 구조화 요약으로 바꾸고, 에이전트 정의 파일의 실제 경로(`.claude/agents/`, 이전 spec의 `frontend/.claude/agents/`는 오기)도 함께 바로잡는다.

## 범위 (Scope)

### In Scope
- `.claude/agents/frontend-reviewer.md` 작성 (프로젝트 루트 — Claude Code가 서브에이전트를 스캔하는 위치)
...
- 통과/실패 처리: plan 체크박스 갱신, `status: done` 갱신 조건, `.claude/backlog/` 규격에 따른 실패 기록
- 대화형 구조화 요약을 통한 결과 보고 (통과/실패, 이슈, 스코프 밖 항목)
- Tools/model 확정

### Out of Scope
- 브라우저/시각/동작(E2E) 검증 — 추후 별도 QA 에이전트로 다룬다
...
- `ReportFindings` 툴 사용 — 커스텀 서브에이전트에 실제로 부여되지 않아 이번 spec에서 제외 (이전 spec의 범위였으나 dry-run으로 불가 확인)

## 사용자 시나리오

1. 사용자가 plan의 한 phase 구현을 마친 뒤 "frontend-reviewer로 `<feature-slug>` Phase `<N>` 검증해줘"라고 호출한다.
2. 에이전트가 `plans/`에서 해당 feature-slug와 매칭되는 plan 파일을 스스로 찾는다.
...
4. plan이 참조하는 spec 파일을 읽어 수용기준 맥락을 파악하고, `.claude/backlog/`에서 같은 feature 또는 관련 category의 과거 실패 항목을 훑어 참고한다.
...
8. 결과를 종합한다.
   - correctness 버그 또는 build/lint 실패가 있으면 phase 실패로 처리하고, `.claude/backlog/`에 `backlog-format.md` 규격대로 기록한다(`category`는 실패 원인에 따라 판단). 이 경우 plan 체크박스는 갱신하지 않는다.
...
10. 검증 결과(통과/실패, 발견된 이슈, 스코프 밖으로 남긴 항목 목록)를 대화형 텍스트로 구조화해 보고한다. 어떤 경우에도 소스 코드는 직접 수정하지 않는다.
...
- **응답**: 대화형 구조화 요약 (통과/실패 여부, correctness/simplification/efficiency 이슈, 스코프 밖 수동 검증 항목 목록). 부수효과로 plan 파일이 갱신되거나(체크박스, `status`) `.claude/backlog/`에 새 파일이 생성될 수 있다.
- **에러 케이스**:
  - `plans/`에 매칭되는 feature-slug plan이 없음 → 중단, spec/plan부터 진행하라고 안내
...
- [ ] `.claude/agents/frontend-reviewer.md` 파일이 프로젝트 루트에 존재하고 frontmatter(`name`, `description`, `tools: Read, Glob, Grep, Bash, Edit, Write`, `model: inherit`)를 포함한다
- [ ] "frontend-reviewer로 `<feature-slug>` Phase `<N>` 검증해줘" 형태 호출 시 `plans/`에서 해당 feature-slug의 plan 파일을 스스로 찾아낸다
...
- [ ] correctness 버그 또는 build/lint 실패가 있으면 phase를 실패 처리하고 `.claude/backlog/` 규격대로 기록하며, plan 체크박스를 갱신하지 않는다
