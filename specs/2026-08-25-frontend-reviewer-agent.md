...
- **응답**: `ReportFindings` 결과(통과/실패 여부, correctness/simplification/efficiency 이슈, 스코프 밖 수동 검증 항목 목록). 부수효과로 plan 파일이 갱신되거나(체크박스, `status`) `.claude/backlog/`에 새 파일이 생성될 수 있다.
- **에러 케이스**:
