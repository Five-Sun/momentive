...
export default function CheckoutPage() {
  ...
  const loadCoupons = useCallback(() => {
    return fetchMyCoupons()
      .then((list) => {
        setCoupons(
          list.filter((c) => c.status === "AVAILABLE" && !isExpired(c)),
        );
  ...
  const couponEligible =
    selectedCoupon !== null && itemsSubtotal >= selectedCoupon.minOrderAmount;
  const discountAmount =
    selectedCoupon && couponEligible
      ? calculateCouponDiscount(selectedCoupon, itemsSubtotal)
      : 0;
  ...
  async function onSubmit(values: AddressFormValues) {
    ...
    try {
      ...
      const userCouponId =
        selectedCoupon && couponEligible ? selectedCoupon.id : undefined;
      ...
      if (err instanceof ApiError) {
        ...
        if (err.errorCode === "VALIDATION_FAILED") {
          const addressFieldErrors = Object.entries(
            err.fieldErrors ?? {},
          ).filter(([field]) => field.startsWith("address."));
  ...
  return (
    <div className="bg-canvas relative flex min-h-screen flex-col">
      <div className="border-hairline bg-surface-card flex h-13 flex-shrink-0 items-center px-4 border-b lg:hidden">
        ...
        <span className="text-title-sm text-ink flex-1 text-center">
          주문서 작성
        </span>
      ...
      <div className="hidden px-0 pt-7 pb-4 lg:block">
        <span className="text-title-sm text-ink">주문서 작성</span>
      </div>

      <form
        onSubmit={handleSubmit(onSubmit)}
        className="flex flex-1 flex-col gap-6 px-4 py-5 pb-28 lg:grid lg:grid-cols-[1fr_340px] lg:items-start lg:gap-10 lg:px-0 lg:py-0 lg:pb-16"
      >
            ...
            {addressesLoaded && addresses.length > 0 && (
              <div className="flex flex-col gap-2">
                {addresses.map((address) => {
                  const checked =
                    !showNewAddressForm && selectedAddressId === address.id;
                  return (
                    <button
                      type="button"
                      key={address.id}
                    ...
                    >
                      ...
                      >
                        {checked && (
                          <Check
                            className="h-3.5 w-3.5 text-on-brand"
                            strokeWidth={3}
                          />
                        )}
                      </span>
                ...
                {coupons.map((coupon) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={!eligible}
                      onClick={() =>
                        setSelectedCouponId(checked ? null : coupon.id)
                      }
                      className={`flex items-start gap-3 rounded-md border p-3 text-left ${
                        checked ? "border-brand-pink" : "border-hairline"
                      } ${eligible ? "" : "opacity-50"}`}
                    >
                      ...
                      <div className="flex flex-col gap-0.5">
                        ...
                        {!eligible && (
                          <span className="text-caption text-muted">
                            {formatWon(coupon.minOrderAmount)} 이상 구매 시 사용
                            가능
                          </span>
                        )}
                      </div>
        ...
        <section className="border-hairline bg-surface-soft mx-4 my-4 flex flex-col gap-2 rounded-md border p-5 lg:sticky lg:top-8 lg:col-start-2 lg:mx-0 lg:my-0">
