...
`plan`의 Phase 검증이 실패했을 때, reviewer가 실패 원인과 해소 방법을 기록하는 문서의 규격이다. plan 파일 자체는 순수 계획 문서로 유지하고(실패 이력을 남기지 않음), 실패로부터 배운 것은 이 별도 문서에 쌓아 같은 실수의 반복을 줄인다.
...
reviewer가 실패 감지 직후 작성하는 경우에는 아직 소스 코드를 고치지 않았으므로, 이 실패를 해소하려면 무엇을 어떻게 바꿔야 하는지 구체적인 권장 조치를 서술한다. 구현자나 후속 작업자가 실제로 수정한 뒤 backlog를 갱신하는 경우에는 실제 수정 내용을 덧붙일 수 있다.
