...
momentive는 GitHub 호스팅 리포지토리라 PR 오픈이 Buzz 이벤트로 자동 발생하지 않는다. `Reviewer`는 상시 감시가 아니라 Buzz 채널 태그로만 실행되므로, PR을 열면 아래 3가지를 모두 충족해야 리뷰가 시작된다.

1. **Welcome 채널에서** `Reviewer`를 태그한다 (세션이 채널 단위라 다른 채널 태그는 감지되지 않음)
2. 메시지에 **PR 링크(GitHub URL)** 를 포함한다 (링크가 없으면 리뷰 대상을 특정할 수 없다)
3. **PR을 연 사람이 직접** 태그한다 (그 feature의 마지막 phase를 마친 쪽 — 위 "PR 단위" 기준. 제3자가 대신 전달하면 대상 브랜치/커밋 특정이 늦어진다)

문구 예시: "PR을 열면 같은 브랜치·같은 채널에서 Reviewer를 태그하고 PR 링크를 남긴다."
